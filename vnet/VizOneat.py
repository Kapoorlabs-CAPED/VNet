    ...
    def VizActivations(self):
        ...
        for count, activation in enumerate(self.activations):
            ...
            if len(max_activation.shape) == 4:
               max_activation_new = np.pad(max_activation, ((0,0),(0,0),(max_activation.shape[-2],self.pad_width[0]), (max_activation.shape[-1],self.pad_width[1])))
            if len(max_activation.shape) == 3:
                max_activation_new = np.pad(max_activation, ((0,0),(max_activation.shape[-2],self.pad_width[0]), (max_activation.shape[-1],self.pad_width[1])))
            if len(max_activation.shape) == 2:
                max_activation_new = np.pad(max_activation, ((max_activation.shape[-2],self.pad_width[0]), (max_activation.shape[-1],self.pad_width[1])))
